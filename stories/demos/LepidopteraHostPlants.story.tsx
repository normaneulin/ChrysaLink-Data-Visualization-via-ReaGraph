...
import { GraphCanvas, lightTheme, Label, Ring } from '../../src';
import {
  buildDivisionLevel,
  ...
  plantSpecies,
  interactions
} from '../assets/lepidoptera-demo';
import { getNodesFor, buildEdgesBetween } from '../assets/import_sample';

export default {
  title: 'Demos/Lepidoptera ↔ Host Plants',
...
export const Interactive = () => {
  ...
  const [leftLevel, setLeftLevel] = useState<'Division' | 'Family' | 'Genus' | 'Species'>('Division');
  ...
  const combinedNodesEdges = useMemo(() => {
    ...
    // Keep the drill scoped to the last clicked division on each side
    const divisionFilterLeft = leftDivision;
    const divisionFilterRight = rightDivision;
    ...
    if (leftStack && leftStack.length > 0) {
      ...
      const levelParent = leftLevel === 'Family' ? divisionFilterLeft : undefined;
    ...
    if (rightStack && rightStack.length > 0) {
      ...
      const levelParent = rightLevel === 'Family' ? divisionFilterRight : undefined;
